...
namespace COM_DoorsLibrary
{
    public sealed class KV11 : KVD
    {
        ...
        public override double LL_Width => Data.Width + 144 - 8;
            ...
            : base(data, cons)
        {
            ...
            Parts.Add(new KVDPartInfo(Command_KVD.Лицевой_лист, "[СТ-КВ11]_Лицевой_лист.SLDPRT", $"{template}_LL"));
            Parts.Add(new KVDPartInfo(Command_KVD.Монтажный_профиль_нижний, "[СТ-КВ11]_Монтажный_уголок.SLDPRT", $"{template}_M"));
            Parts.Add(new KVDPartInfo(Command_KVD.Верхний_профиль, "[СТ-КВ11]_Торцевой_профиль.SLDPRT", $"{template}_T"));
            Parts.Add(new KVDPartInfo(Command_KVD.Замковая_стойка, "[СТ-КВ11]_Стойка_ЗС_ПС.SLDPRT", $"{template}_{GetNalichikKod(GetNalichnik())}_{GetPositionKod()}_ZS"));
            Parts.Add(new KVDPartInfo(Command_KVD.Петлевая_стойка, "[СТ-КВ11]_Стойка_ЗС_ПС.SLDPRT", $"{template}_{GetNalichikKod(GetNalichnik(false))}_{GetPositionKod(false)}_PS"));
            Parts.Add(new KVDPartInfo(Command_KVD.Притолока, "[СТ-КВ11]_Притолока.SLDPRT", $"{template}_{GetNalichikKod(Data.Nalichniki[(int)Raspolozhenie.Верх])}_US"));
            Parts.Add(new KVDPartInfo(Command_KVD.Порог, "[СТ-КВ11]_Порог.SLDPRT", $"{template}_{GetPorogKod()}"));
            Parts.Add(new KVDPartInfo(Command_KVD.РЖ_полотна, "[СТ-КВ11]_RZP.SLDPRT", $"{template}_RZP"));
        }
    }
}
